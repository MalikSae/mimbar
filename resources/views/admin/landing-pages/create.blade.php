...
@section('content')
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8" x-data="landingPageForm()">
    <div class="mb-6">
        <a href="{{ route('admin.landing-pages.index') }}" class="text-primary hover:text-primary-dark text-sm font-medium">&larr; Kembali</a>
    </div>

    <h1 class="text-2xl font-bold font-heading text-gray-900 mb-6">Buat Landing Page Baru</h1>
        ...
        <form action="{{ route('admin.landing-pages.store') }}" method="POST">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" x-model="title" required class="mt-1 block w-full rounded-lg border-border focus:border-primary focus:ring-primary shadow-sm sm:text-sm" value="{{ old('title') }}">
                ...
                <div>
                    ...
                    <div class="mt-1 flex rounded-md shadow-sm">
                        ...
                        <input type="text" name="slug" id="slug" x-model="slug" required class="block w-full min-w-0 flex-1 rounded-none rounded-r-lg border-border focus:border-primary focus:ring-primary sm:text-sm" value="{{ old('slug') }}">
                    </div>
                ...
                <div>
                    ...
                    <div class="space-y-4 sm:flex sm:items-center sm:space-y-0 sm:space-x-10">
                        <div class="flex items-center">
                            <input id="canvas_mode_full_canvas" name="canvas_mode" type="radio" value="full_canvas" {{ old('canvas_mode', 'full_canvas') == 'full_canvas' ? 'checked' : '' }} class="h-4 w-4 border-gray-300 text-primary focus:ring-primary">
                        ...
                        <div class="flex items-center">
                            <input id="canvas_mode_full_page" name="canvas_mode" type="radio" value="full_page" {{ old('canvas_mode') == 'full_page' ? 'checked' : '' }} class="h-4 w-4 border-gray-300 text-primary focus:ring-primary">
                ...
                <div>
                    ...
                    <input type="text" name="meta_title" id="meta_title" class="mt-1 block w-full rounded-lg border-border focus:border-primary focus:ring-primary shadow-sm sm:text-sm" value="{{ old('meta_title') }}">
                ...
                <div>
                    ...
                    <textarea name="meta_description" id="meta_description" rows="3" class="mt-1 block w-full rounded-lg border-border focus:border-primary focus:ring-primary shadow-sm sm:text-sm">{{ old('meta_description') }}</textarea>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('landingPageForm', () => ({
            title: '{{ old('title') }}',
            slug: '{{ old('slug') }}',
            isSlugEdited: false,
            init() {
                if (this.slug && this.title) {
                    let expectedSlug = this.title.toLowerCase().replace(/[^a-z0-9 -]/g, '').replace(/\s+/g, '-').replace(/-+/g, '-');
