...
<form method="GET" action="{{ route('admin.kategori.index') }}"
    ...
    <select name="type"
            style="padding: 8px 14px; border: 1px solid var(--color-border); border-radius: var(--radius-lg); font-size: 14px; font-family: var(--font-body); outline: none; background: white;">
    ...
    <table style="width: 100%; border-collapse: collapse;">
        <thead>
            <tr style="background: var(--color-muted);">
                ...
                <th style="padding: 12px 20px; text-align: left; font-size: 11px; font-weight: 500; color: var(--color-gray-600); text-transform: uppercase; width: 140px;">Tipe</th>
                <th style="padding: 12px 20px; text-align: center; font-size: 11px; font-weight: 500; color: var(--color-gray-600); text-transform: uppercase; width: 120px;">Jumlah Data</th>
        ...
        <tbody>
            ...
            <tr style="border-bottom: 1px solid var(--color-border);">
                ...
                <td style="padding: 14px 20px;">
                    @if($cat->type === 'news')
                    <span style="font-size: 11px; padding: 3px 10px; border-radius: var(--radius-full); font-weight: 500; background: #dbf4ff; color: #0284c7;">Berita</span>
                    @else
                    <span style="font-size: 11px; padding: 3px 10px; border-radius: var(--radius-full); font-weight: 500; background: var(--color-primary-light); color: var(--color-primary);">Artikel</span>
                    @endif
                </td>
                ...
                <td style="padding: 14px 20px; text-align: center;">
                    <div style="display: flex; gap: 6px; justify-content: center;">
                        <button type="button" @click="openEdit({{ $cat->id }}, '{{ addslashes($cat->name) }}', '{{ $cat->type }}')"
                        ...
                        <form method="POST" action="{{ route('admin.kategori.destroy', $cat->id) }}" x-data @submit.prevent="if(confirm('Yakin hapus kategori ini? Artikel/Berita di dalamnya akan diset Uncategorized.')) $el.submit()">
                            @csrf @method('DELETE')
                            <button type="submit"
            ...
            @empty
            <tr>
                <td colspan="4" style="padding: 48px 20px; text-align: center; font-size: 14px; color: var(--color-gray-400);">Belum ada kategori.</td>
            </tr>
            @endforelse
        </tbody>
    </table>
...
<div x-show="showModal" x-cloak style="position:fixed;top:0;left:0;right:0;bottom:0;z-index:9999" @keydown.escape.window="showModal=false">
    <div style="position:absolute;top:0;left:0;right:0;bottom:0;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.5)" @click.self="showModal=false">
        <div style="background:#fff;border-radius:var(--radius-xl);padding:24px;width:400px;max-width:90vw;box-shadow:0 24px 64px rgba(0,0,0,.22)">
            <h3 style="font-family:var(--font-heading);font-size:18px;font-weight:700;color:var(--color-gray-900);margin:0 0 16px;" x-text="editMode ? 'Edit Kategori' : 'Tambah Kategori'"></h3>
            ...
            <form :action="formAction" method="POST">
                ...
                <div style="margin-bottom: 24px;">
                    <label style="display:block;font-size:13px;font-weight:500;color:var(--color-gray-700);margin-bottom:6px">Fungsi / Tipe</label>
                    <select name="type" x-model="categoryType" required
                            style="width:100%;padding:10px 14px;box-sizing:border-box;border:1px solid var(--color-border);border-radius:var(--radius-lg);font-size:14px;font-family:var(--font-body);outline:none;background:white;">
